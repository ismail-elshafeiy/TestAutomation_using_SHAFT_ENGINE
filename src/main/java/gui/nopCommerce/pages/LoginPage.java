...
public class LoginPage {
    ...
    public LoginPage selectRememberMe_ChkBx(String state) {
        if (state.equalsIgnoreCase("uncheck") && driver.element().get().isDisplayed((rememberMe_ChkBx))) {
            driver.element().click(rememberMe_ChkBx);

        } else if (state.equalsIgnoreCase("check") && !driver.element().get().isDisplayed(rememberMe_ChkBx)) {
            driver.element().click(rememberMe_ChkBx);
        }
        return this;
    }
